      ...
      <div className="personal--container">
        ...
        <div className="personal--main">
          <div className="personal--infor">
            <div className="personal--avatar--name">
              ...
              <div className="personal--name">
                <h1 className="personal--username">
                  {localStorage.getItem("username")}
                </h1>
                <h1 className="personal--fullname">
            ...
            <div className="personal--counts">
              <p>
                <span>{fixCount(following)}</span> {props.lang("user.following")}
              </p>
              <p>
                <span>{fixCount(follower)}</span> {props.lang("user.follower")}
              </p>
              <p>
                <span>{fixCount(like)}</span> {props.lang("user.likes")}
              </p>
            </div>
            <Des />
          </div>
          <div className="personal--video">
            ...
            >
              {props.lang("user.myvideo")}
            </button>
            <button
              className={change_color2}
              onClick={() => {
                setTrans("trans--1");
          ...
          <div className={myvideos}>
            {videos.length != 0 ? (
              videos.map((val, key) => (
                <Link className="video--link" to={"personal/video/" + val.id}>
                  <VideoChild val={val} />
                </Link>
              ))
            ) : (
              <div className="null--videos">
                <img
                  src="https://res.cloudinary.com/diw0u2vl1/image/upload/v1642019466/assets/aa1_ig429l.png"
                  alt=""
                />
          ...
          <div className={mylikes}>
            {video_likes.length != 0
              ? video_likes.map((val, key) => (
                  <Link
                    className="video--link"
                    to={"personal/video/" + val.video.id}
                  >
                    <VideoChild val={val.video} />
                  </Link>
                ))
              : null}
          </div>
